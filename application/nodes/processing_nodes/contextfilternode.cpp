...
filterReturnValue_t ContextFilterNode::filterData(CircularBuffer* buffer, CircularBufferReader *bufferReader)
{
    ...
    if((settings->getMergeMode() == MergeMode_t::NO_CONTROL)||(amountOfSubscriptions == 1))
    {
        ...
        //contextFilterEngine->filterData(lambda, bufferReader, bufferReader->availableSize(), getBufferUnusedSize(),&metaData);
...
UpdateReturn_t ContextFilterNode::NotifyBufferUpdate(Subscription *source)
{
    ...
    filterReturnValue_t filterValue = filterData(circularBuffer, source->bufferReader);


    UpdateReturn_t updateState = notifyAllSubscriptions();
